...
`1.0.0-rc.6`. **Findings 1–10 are fixed on branch `rc7`**, one commit each,
each pinned by the test its entry names; 11 is deferred to 1.1 as the plan
says. The entries below are kept as written — the reasoning is the useful
part — with a *Status* line on each.

Alongside them, one change the plan did not list: the return annotation is
`: T`, not `-> T` (`E0906`, with a `fix`). It came out of reviewing the
plan's own code sample.
...
|---|---|---|---|---|
| 1 | todo CRUD, e-school | `boolean(x)` answers `false` for everything it does not recognise | wrong status | fixed |
| 2 | todo CRUD, e-school | `enum(E, x)` never reads `E` | wrong status | fixed |
| 3 | e-school | `jwc fmt --check` and `jwc fmt` report each other's outcome | ergonomics | fixed |
| 4 | e-school | `jwc explain` lists no write statement | ergonomics | fixed |
| 5 | MyWallet, shortener | an absent `jwc` field means "any version", so the projects that need the check never get it | ergonomics | fixed |
| 6 | MyWallet, shortener | one `--` comment produces one diagnostic per character on the line | ergonomics | fixed |
| 7 | MyWallet, shortener | rc.3's write binder shipped without a migration diagnostic | ergonomics | fixed — `E0907` |
| 8 | MyWallet, shortener | 116 diagnostics carried a machine-applicable fix and nothing applies them | ergonomics | fixed — `jwc fix` |
| 9 | MyWallet, shortener | `jwc fmt` cannot format a file with a comment inside `server { }` | ergonomics | fixed |
| 10 | reading the language | `after` runs *before* the response is sent, and only a write can be taken off that path | naming, gap | docs fixed; gap waits for 11 |
| 11 | shortener | nothing in the language runs two things at once inside one request | gap | deferred to 1.1 |
...
**Proposal.** Parse `true` / `false` — only those two, because that is what
`src/serve.rs:617` accepts, and `?done=1` answering `true` while `/x/1`
answers 404 is the same two-door split again — and raise `BadRequest`
otherwise, the way `date` does.

The null case is decided by the neighbours. `int(null)` is `""` → *"is not
a number"* → `BadRequest` (`src/exec_call.rs:629`), and `date(null)` raises
the same way. So `boolean(null)` raises too, and `builtins.md §2` says so.
The alternative — `boolean(x)` returning `boolean?` so that null-in gives
null-out — reads better at a call site, but it would make `boolean` the
one coercion that propagates `?` while `int`, `date`, `uuid` and
`timestamptz` do not. If `T? → X?` is wanted it is one rule for every
coercion, and that is a checker change with its own entry, not part of
this fix. The `enum(E, x)` precedent does not transfer: it is declared `E?`
in the table already, and its null half is the half that works.

**Pinned by:** `boolean("bogus")` → `BadRequest`; `boolean(null)` →
`BadRequest`; `boolean("1")` → `BadRequest`; route `{x: boolean}` and
`boolean(x)` accept the same two strings.

**Status:** fixed, `tests/coercions.rs`, both backends. The route parameter answers 400 (`bad_path_parameter`), not 404 as written above.

**Workaround today**, and what the todo app ships:

```jwc
function flag(raw: text?) {
    if (@raw == null) {
        return null;
    }
...
The member list is not missing, and it is not out of reach either.
`src/check.rs:2795` resolves `E` against `self.sym.enums` to type the call,
and the interpreter reads the same table — `self.program.symbols.enums` at
`src/exec.rs:1337`. Nothing has to be carried anywhere; the arm at line 60
simply never looks.

**Proposal.** Look `E` up in `self.program.symbols.enums` and raise
`BadRequest` naming the value and the members. The checker already proves
`E` exists, so the lookup cannot fail.

**Pinned by:** `enum(Priority, "bogus")` → `BadRequest` naming the members;
`enum(Priority, null)` → `null`; the sample's
`/api/v1/orgs/1/invoices?status=bogus` → 400.

**Status:** fixed, `tests/coercions.rs` and `tests/native.rs`; the native backend writes the members into the call.
...
**Pinned by:** `fmt --check` on a clean tree prints `already formatted`;
`fmt` on a clean tree prints `formatted`.

**Status:** fixed, `tests/tooling.rs`.
...
**Pinned by:** `explain` over a program with one `select`, one `insert`,
one `update` and one `delete` lists four sites, and the `update` site shows
its `WHERE … ctid = (… FOR UPDATE LIMIT 1)` clause.

**Status:** fixed, `tests/tooling.rs`. The clause is by primary key since rc.6, not `ctid`: `WHERE x.id = (SELECT y.id … FOR UPDATE LIMIT 1)`. A spread prints the fields its parameter's class carries; hover answers over a write too. The sample's 50 statements all compile.
...
**Pinned by:** a manifest without `jwc` plus one `E0901` prints the header
once, before the list; the same manifest with no `E090x` prints nothing
extra.

**Status:** fixed, `tests/language_version.rs`.
...
The fix is not where the message is. `E0901` is a **parser** diagnostic
(`src/parser.rs:137` and `:320`, on seeing `Minus Minus` where a
declaration or statement should start), but `E0100` is a **lexer**
diagnostic (`src/lexer.rs:331`). The lexer does not know `--` — it emits
two `Tok::Minus` and carries on tokenising the prose, so all 89 `E0100`s
are already in the list before the parser ever reaches the `--` that
explains them. Nothing the parser does afterwards can take them back, and
the prose words it does receive become identifiers it then trips over too.

**Proposal.** Teach the lexer: `--` at the start of a line (after
whitespace) is the old comment marker — record one `E0901` there, skip to
the newline, and emit nothing for the line. The parser's two `E0901` sites
stay for `--` that appears mid-line, which the lexer must leave alone
because `a -- b` is `a - (-b)`. One `E0901` per line, no `E0100`s under
it, and the reader sees the 29 real ones.

**Pinned by:** a file whose one line is ``-- it's `x` — see §2`` produces
exactly one diagnostic, `E0901`.

**Status:** fixed, `tests/removed_keywords.rs`.
...
recognisable at the parse site. Each one carries the replacement text
finding 8 needs — this item lands before 8, because `jwc fix` can only
apply a diagnostic that exists.

**Pinned by:** `insert into T` produces one diagnostic, not three, and it
names `insert T into T`.

**Status:** fixed as `E0907`, `tests/removed_keywords.rs`. The binder offered is the table's own name. `grammar.ebnf` had never been updated for the rc.3 binder; it has now.
...
carries a replacement, repeat until the count stops falling, print what it
changed. The `--help` line writes itself: *the migrations the compiler
already knows how to do.*

What it must **not** do is read the `help:` string. `Diagnostic` has one
free-text field for it, `note: Option<String>` (`src/diag.rs:33`), and not
every ``write `X` `` in it is a replacement. `E0301`'s note is ``write
`enum(InvoiceStatus, request.query("status"))` `` (`src/check.rs:2808`) —
an *example*, and a `fix` that parsed notes would overwrite the author's
`enum(...)` call with the sample. ``write `true` or `false` ``
(`src/check.rs:1819`) is two alternatives. The script that fixed 116 sites
got away with it because a person had chosen `E0904` first.

So: a second field, `fix: Option<String>`, holding the literal text that
replaces `span`, set only where the compiler has made the whole decision —
`E0901` (`//`), `E0903` (`@name`), `E0904` (`Driving.col`), and the
rc-series diagnostics from finding 7. `jwc fix` applies those and nothing
else; `help:` stays prose. Every diagnostic that gains a `fix` gets the
same review a code change does, because it now edits the user's file.

**Pinned by:** a tree with one of each fixable diagnostic checks clean
after one `jwc fix`; a tree with only `E0301` is left untouched.

**Status:** fixed, `tests/tooling.rs`. `Diagnostic.fix` is set on `E0901`, `E0902`, `E0903`, `E0904` (one candidate), `E0906`, `E0907`. `jwc fix` does not refuse a project pinned to another release, and says at the end what the field should become.
...
**Pinned by:** `fmt` round-trips a `///` line inside `server { }`, inside a
record literal and inside an `insert` value list.

**Status:** fixed, `tests/fmt.rs`. An array item still carries no comment; the refusal for that case now says it is a gap being tracked.
...
**Status:** the documentation half is done — middleware.md §5 opens with the position and the measurement, and `E0811` says it. The gap waits for 11.
...
Every item above names what pins it; an item is done when that test is in
the tree, not when the code changes.

1. **3** — two strings, swapped back. Five minutes, and it stops costing
   every reader of CI output a double-take from here on.
2. **1 and 2** — the only two that produce a wrong answer or a wrong status
   to a live client. Small, and they close a hole the sample shares. 2 is
   a lookup in a table the interpreter already holds.
3. **6** — one arm in the lexer removes a third to two thirds of what a
   migrating reader has to read.
4. **7, then 5** — name the rc-series removals the way the 0.9 ones are
   named, each carrying its replacement; then say which release the source
   is from. 7 goes before 8 because 8 can only apply what 7 emits.
5. **8** — `jwc fix`, on a `fix` field and not on prose. It pays for itself
   on the next release that moves anything, and both ports above were
   dress rehearsals for it.
6. **9** — comments in the three positions `fmt` cannot hold.
7. **4** — `jwc explain` over writes. The largest of the defects, and the
   one with a defect of its own already charged to it.
8. **10**, the documentation half only — one sentence in middleware.md §5
