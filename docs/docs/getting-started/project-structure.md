...
A bare version means exactly that version. While the 1.0 candidates are
...
admit `1.0.0-rc.6`, while `^1.0.0-rc.1` does.

```console
$ jwc check
Error: this is jwc 1.0.0-rc.6, and ./jwcproj.json says the project is
written for `1.0.0-rc.4`.

Install the version it asks for, or — once the source has been moved to
this one — change `jwc` in that file to `1.0.0-rc.6`. Diagnostics from
