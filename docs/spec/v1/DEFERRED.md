...
| `DEFERRED-8` | Multi-row `insert` | `for (let x in xs) { insert T into … }` inside a `transaction` (writes §2.1) | One statement shape, one RETURNING shape. Batching is a performance change with no new semantics |
| `DEFERRED-9` | `SKIP LOCKED` / work claiming | `update … first` always emits `FOR UPDATE` (writes §4) | Work-claiming is a queue feature, and the queue now exists — but it is the runtime's, not the language's: `_jwc_jobs` claims with `FOR UPDATE SKIP LOCKED` (jobs §3.3) and a program cannot write that clause itself. What stays deferred is the query-language surface |
